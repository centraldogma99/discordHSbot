...
export class Paginator {
  ...
  constructor(
    ...
    */
    ...
    if (!Array.isArray(promises.value))
    ...
    if (lengthEnabled && !length)
      throw Error(
        "Paginator: if @lengthEnabled is true, @length must be given"
      );
    if (promises.isPromise && !promiseResSize)
      throw Error(
        "Paginator: if @isPromise is true, @promiseResSize must be given"
      );
  ...
  async next() {
    ...
    // this.cards 장전하기
    while (
      this.cursor + this.paginateStep > this.images.length &&
      (this.promises.value.length > 0 || this.nextPagePromise)
    ) {
      if (this.promises.isPromise) {
        ...
        const promiseUnitSize = Math.ceil(
          this.paginateStep / this.promiseResSize
        );
        ...
        if (!this.nextPagePromise) {
          ...
          images = await Promise.all(
            reqIdsCurrent.map((reqId) => RequestScheduler.getRes(reqId))
          );
          // 배열의 길이를 넘어가서 slice를 하더라도 정상동작(빈 배열이 됨)
          this.promises.value = this.promises.value.slice(
            numOfPromisesNeedToResolved
          );
        } else {
        ...
        if (this.promises.value.length > 0) {
          ...
          if (numOfPromisesNextPage > this.promises.value.length)
            numOfPromisesNextPage = this.promises.value.length;
          let reqIdsNext = Array(numOfPromisesNextPage).fill(null);
          reqIdsNext = reqIdsNext.map((_, index) =>
            RequestScheduler.addReq((this.promises as any).value[index])
          );
          this.nextPagePromise = Promise.all(
            reqIdsNext.map((reqId) => RequestScheduler.getRes(reqId))
          );
          this.promises.value = this.promises.value.slice(
            numOfPromisesNextPage
          );
        }
      } else {
