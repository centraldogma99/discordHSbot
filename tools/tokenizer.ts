...
import korClass from '../languages/kor/class.json';
import engClass from "../languages/eng/class.json"
...
export function tokenizer(msgContent: string, languageMode: string): Tokens {
  ...
  function parseConditions(conditions: string[]): Condition {
    ...
    let ret: Condition = {};
    const lang = languageMode === "ko_KR" ? korClass : engClass;

    function isValidClass(condition: string) {
      if (lang
        .map(c => c.string)
        .reduce((f, s) => [...f, ...s])
        .includes(condition)) return true;
      else return false;
    }
    ...
    conditions.forEach(condition => {
      ...
      if (isValidClass(condition)) {
        const retcls = lang.filter(cls => cls.string.includes(condition))[0];
