...
async function deck(message: Message, args: string) {
  ...
  try {
    deckInfo = await RequestScheduler.getRes(
      RequestScheduler.addReq(deckInfoPromise)
    );
  ...
  let cards: Card[] = deckInfo.cards.sort(
    (a: Card, b: Card) => a.manaCost - b.manaCost
  );
  let names = cards.map((card) => card.name);
  let costsAndRarities = Object.fromEntries(
    cards.map((card) => [
      card.name,
      { cost: card.manaCost, isLegendary: card.rarityId == 5 ? "⭐" : "" },
    ])
  );
  let obj = {};
  ...
  const str = Object.keys(obj)
    .map(
      (k) =>
        `${obj[k]} x (${costsAndRarities[k].cost}) ${k} ${costsAndRarities[k].isLegendary}`
    )
    .join("\n");
  ...
  const pagi = new Paginator(
    message,
    { value: cards.map((card: Card) => card.image), isPromise: false },
